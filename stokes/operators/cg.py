...
class AdvectionOperatorP1(NumpyMatrixBasedOperator):
    ...
    def _assemble(self, mu=None):
        ...
        VSF_GRAD = P1ShapeFunctionGradients(g.dim)
        del q
...
class L2VectorProductFunctionalP2(NumpyMatrixBasedOperator):
    ...
    def _assemble(self, mu=None):
        ...
        if bi is not None and bi.has_dirichlet:
            ...
            # edge dirichlet nodes
            EDN = bi.dirichlet_boundaries(g.dim - 1) + g.size(g.dim)
            #EDN2 = bi.dirichlet_boundaries(g.dim - 1)
            # dirichlet nodes
            ...
            if self.dirichlet_data is not None and not self.clear_dirichlet_dofs:
                ...
                #DC2 = np.concatenate((g.centers(g.dim)[VDN], g.centers(g.dim - 1)[EDN2]))

                # evaluate dirichlet function
                #c = np.einsum('ij,ej->ei', mu['transformation'], DC)
                D = self.dirichlet_data(DC, mu=mu)
...
class TransposedOperator(NumpyMatrixBasedOperator):
    ...
    def projected(self, range_basis, source_basis, product=None, name=None):
        proj_operator = self.operator.projected(range_basis=range_basis, source_basis=source_basis, product=product)

        return super(TransposedOperator).__init__(proj_operator)