...
def discretize_gmsh(domain_description=None, geo_file=None, geo_file_path=None, msh_file_path=None,
                    mesh_algorithm='del2d', clscale=2., options='', refinement_steps=0):
    ...
    def discretize_PolygonalDomain():
        ...
        inner_edges = domain_description.inner_edges

        return points, domain_description.boundary_types, inner_edges
    ...
    try:
        ...
        if domain_description is not None:
            ...
            if isinstance(domain_description, PolygonalDomain):
                points, boundary_types, inner_edges = discretize_PolygonalDomain()
            ...
            # write inner edges as spline into the file
            for i in xrange(len(inner_edges)):
                inner_edges[i] = [x+1 for x in inner_edges[i]]
            inner_edge_ids = range(len(lines)+1, len(lines)+len(inner_edges)+1)
            for ie_id, ie in zip(inner_edge_ids, inner_edges):
                geo_file.write('Spline('+str(ie_id)+')'+' = '+str(ie).replace('[', '{').replace(']', '}')+';\n')
            ...
            line_loop_ids = range(len(lines)+len(inner_edges)+1,len(lines)+len(inner_edges)+len(line_loops)+1)
            ...
            for boundary_type, bs in boundary_types.iteritems():
                geo_file.write('Physical Line' + '("' + str(boundary_type) + '")' + ' = '
                               + str([l_id+1 for l_id in bs]).replace('[', '{').replace(']', '}') + ';\n')
            # line in surface
            for ie_id, ie in zip(inner_edge_ids, inner_edges):
                geo_file.write('Line {'+str(ie_id)+'}'+' In Surface {' + str(line_loop_ids[0]+1) + '};\n')
